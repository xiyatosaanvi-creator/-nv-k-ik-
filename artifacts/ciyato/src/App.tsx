...
type ScreenEntry = { id: string; component: ReactElement; light?: boolean };
type NavGroup = { label: string; screens: ScreenEntry[] };

const groups: NavGroup[] = [
  { label: "Onboarding", screens: [{ id: "Onboarding", component: <Onboarding /> }] },
  {
    label: "Launcher layer",
    screens: [
      { id: "Home", component: <Home /> },
      { id: "App Drawer", component: <AppDrawer />, light: true },
    ],
  },
  {
    label: "Internal app layer",
    screens: [
...
export default function App() {
  ...
  const active = allScreens.find((s) => s.id === activeScreen) || allScreens[0];
      ...
      <div style={{ display: "flex", height: "100vh", width: "100vw", background: "#06080A", color: "#F4F5F6", fontFamily: "Inter, system-ui, sans-serif", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 12px", scrollbarWidth: "none" }}>
            {groups.map((group) => (
              <div key={group.label} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", color: T.muted, padding: "10px 12px 4px" }}>
                  {group.label}
                </div>
                {group.screens.map((s) => {
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative" }}>
          ...
          <div
            ...
            style={{ position: "absolute", top: 20, left: 20, zIndex: 50, width: 36, height: 36, borderRadius: 18, background: T.cardMed, border: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.3s", backdropFilter: "blur(10px)" }}
          >
