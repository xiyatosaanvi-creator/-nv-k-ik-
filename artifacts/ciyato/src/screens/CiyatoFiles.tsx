...
export const CiyatoFiles = () => {
  const [tab, setTab] = useState<"overview" | "browse">("overview");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: T.bg, fontFamily: "Inter, system-ui, sans-serif", color: T.white }}>
      <StatusBar />
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 18px 0", scrollbarWidth: "none", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", background: T.card, borderRadius: 12, padding: 4, marginBottom: 16 }}>
          {(["overview", "browse"] as const).map((t) => (
            <div
              key={t}
              onClick={() => setTab(t)}
              style={{
                ...
                background: tab === t ? T.cardMed : "transparent",
                color: tab === t ? T.white : T.muted,
                cursor: "pointer",
              }}
            >
              {t === "overview" ? "Overview" : "Browse files"}
            </div>
          ))}
        </div>

        {tab === "overview" ? <FilesOverview /> : <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}><FilesBrowser /></div>}
